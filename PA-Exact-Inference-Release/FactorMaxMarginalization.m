...
B.card = A.card(mapB);
index = 1:prod(B.card);
assignmentsB = IndexToAssignment(index,B.card);
assignmentsA = IndexToAssignment(1:prod(A.card),A.card);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i = 1:size(assignmentsB,1)
    ib = assignmentsB(i,:);
    idx = all(assignmentsA(:,mapB)==ib,2);
    B = SetValueOfAssignment(B,ib,max(A.val(idx)));
end
end
