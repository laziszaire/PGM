...
ngeno = size(genotypesToAlleles,1);
genotypeFactor.card = [ngeno];
% Fill in genotypeFactor.var.  This should be a 1-D row vector.
% Fill in genotypeFactor.card.  This should be a 1-D row vector.

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
Assignment = IndexToAssignment(1:prod(genotypeFactor.card),genotypeFactor.card);
for i = 1:size(Assignment,1)
    geno = Assignment(i,:);
    p = pgeno(geno,genotypesToAlleles,alleleFreqs);  
    genotypeFactor = SetValueOfAssignment(genotypeFactor,geno,p);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
end
