...
% Initialize factors
% We Need 3*numPeople factors 
%     a factor  for the CPD at each of 2 parental copies of the gene
%     a factor for the  CPD at the phenotype
% Note that the order of the factors in the list does not matter.
...
for i=1:numPeople
    parentsi = parents(i,:);
    [c1,c2,pheno] = f(parentsi,i,numPeople,numAlleles,alphaList,alleleFreqs);
    [factorList(i),factorList(numPeople+i),factorList(2*numPeople+i)] = deal(c1,c2,pheno);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
end

end

function [c1,c2,pheno] = f(parents1,personid,numPeople,numAlleles,alphaList,alleleFreqs)
% for a person
if all(parents1>0)
[p1,p2] = deal(parents1(1),parents1(2));
card = numAlleles*[1,numel(p1),numel(p2)];
else
    [p1,p2] = deal([],[]);
    card = numAlleles;
end
...
%copy1
copy1_val = copy_val(card,alleleFreqs);
c1 = struct('var',[personid,p1,p1+numPeople],'card',card,'val',copy1_val);

%copy2
c2 = struct('var',[personid+numPeople,p2,p2+numPeople],'card',card,'val',copy1_val);

%pheno 
pheno = phenotypeGivenCopiesFactor(alphaList, numAlleles,personid, numPeople+personid, 2*numPeople+personid);
end



function  val =  copy_val(card,alleleFreqs)

index = 1:prod(card);
assignments = IndexToAssignment(index,card);
if size(assignments,2)>1
    a1 = assignments(:,1) == assignments(:,2);
    a2 = assignments(:,1) == assignments(:,3);
    val =(a1+a2)/2;
else
    val = alleleFreqs;
end
